 ...
 * At each 5-minute market start, place limit buys for BTC/ETH/SOL/XRP Up and Down at a fixed price (e.g. $0.45).
...
import { createClobClient, verifyClobAuth, type ClobClient } from "./clob.js";
...
import { logLine, flushLogs } from "./logger.js";

const LIMIT_PRICE = 0.45;
const PERIOD_DURATION = 300;
...
async function discoverMarket(
  ...
  for (let i = 0; i < slugPrefixes.length; i++) {
    ...
    if (includePrevious) {
      for (let offset = 1; offset <= 3; offset++) {
        const tryTime = roundedTime - offset * PERIOD_DURATION;
        slug = `${prefix}-updown-5m-${tryTime}`;
        try {
          const market = await api.getMarketBySlug(slug);
          if (!seenIds.has(market.conditionId) && market.active && !market.closed) {
            log(`Found ${name} market by slug: ${market.slug} | Condition ID: ${market.conditionId}`);
  ...
  throw new Error(`Could not find active ${name} 5-minute up/down market (tried: ${slugPrefixes.join(", ")})`);
}
...
async function getOrDiscoverMarkets(
  ...
  const eth = enableEth
    ? await discoverMarket(api, "ETH", ["eth"], now, seenIds, true).catch(() => {
        ...
        return disabledMarket("dummy_eth_fallback", "eth-updown-5m-fallback", "ETH Trading Disabled");
      })
  ...
  const solana = enableSolana
    ? await discoverMarket(api, "Solana", ["solana", "sol"], now, seenIds, false).catch(() => {
        ...
        return disabledMarket("dummy_solana_fallback", "solana-updown-5m-fallback", "Solana Trading Disabled");
      })
  ...
  const xrp = enableXrp
    ? await discoverMarket(api, "XRP", ["xrp"], now, seenIds, false).catch(() => {
        ...
        return disabledMarket("dummy_xrp_fallback", "xrp-updown-5m-fallback", "XRP Trading Disabled");
      })
...
async function main(): Promise<void> {
  ...
  log("✅ Trading enabled for BTC and " + (extras.length ? extras.join(", ") : "no additional") + " 5-minute markets");
  ...
  for (;;) {
    ...
    if (lastSeenPeriod !== snapshot.period_timestamp) {
      ...
      const snapshotRefreshed = await fetchSnapshot(api, ethMarket, btcMarket, solanaMarket, xrpMarket);
      log("📊 " + formatPrices(snapshotRefreshed));
      // Place batch ASAP after detecting new market (one request for all limit orders)
      if (lastPlacedPeriod !== snapshotRefreshed.period_timestamp) {
        lastPlacedPeriod = snapshotRefreshed.period_timestamp;
    ...
    if (slEnabled && !simulation && clobClient && timeElapsed > 2 && !clobAuthInvalid) {
      const markets: Array<{
        name: string;
        conditionId: string;
      ...
      for (const m of markets) {
        ...
        let downBal: number;
        try {
          upBal = await trader.getBalance(m.upTokenId);
          downBal = await trader.getBalance(m.downTokenId);
        } catch (balanceErr) {
          if (balanceErr instanceof Error && balanceErr.message === "POLYMARKET_API_KEY_INVALID") {
            clobAuthInvalid = true;
            log("\n❌ Polymarket API key rejected (401). Update api_key, api_secret, api_passphrase in config.json from https://polymarket.com/settings?tab=builder (use proxy_wallet + signature_type: 2 if using Gnosis Safe). Balance checks disabled for this run.\n");
          }
          break;
        }
