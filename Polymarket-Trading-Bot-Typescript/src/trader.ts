...
export class Trader {
  ...
  async executeLimitBuyBatch(
    ...
    for (const opp of toPlace) {
      const price = Math.round(limitPrice * 100) / 100;
      log.info(`   ${tokenTypeDisplayName(opp.token_type)}: $${price} x ${sharesPerOrder} shares`);
    }

    if (this.simulation) {
      log.info("🎮 SIMULATION MODE - Batch limit orders NOT placed");
      for (const opp of toPlace) {
    ...
    if (allEmpty && results.length > 0) {
      log.warn("⚠️ Batch returned no order IDs - retrying each order individually");
      results = [];
      for (const opp of toPlace) {
        const size = Math.round(sharesPerOrder * 100) / 100;
        const price = Math.round(limitPrice * 100) / 100;
        try {
          ...
          log.error(`   ❌ ${tokenTypeDisplayName(opp.token_type)}: ${String(e)}`);
          results.push({ orderID: "", status: "failed" });
        }
      }
    } else {
      log.info(`✅ BATCH: ${results.length} order(s) sent in one request`);
    }
    let confirmed = 0;
    let notConfirmed = 0;
    for (let i = 0; i < toPlace.length; i++) {
      ...
      if (isConfirmed) {
        confirmed++;
        log.info(`   ✅ ${name} confirmed — Order ID: ${orderId} (${status})`);
      } else {
        notConfirmed++;
        if (errorMsg) {
          log.error(
            `   ❌ ${name} not confirmed — ${errorMsg}${orderId ? ` | Order ID: ${orderId}` : ""}`
          );
        } else {
          log.error(`   ❌ ${name} not confirmed — status: ${status}${orderId ? ` | Order ID: ${orderId}` : " (no ID)"}`);
        }
      }
    }
    if (notConfirmed > 0) {
      log.info(`   📋 Batch result: ${confirmed} confirmed, ${notConfirmed} not confirmed`);
    }
  }
  ...
  async cancelPendingLimitBuy(periodTimestamp: number, tokenId: string): Promise<void> {
    ...
    if (orderId) {
      ...
      log.info(`   🗑️ Cancelled limit order ${orderId}`);
      return;
    }
    ...
    for (const o of orders) {
      if (o.side === "BUY" && (o.asset_id === tokenId || (o as { asset_id?: string }).asset_id === tokenId)) {
        ...
        log.info(`   🗑️ Cancelled limit order ${o.id}`);
        return;
      }
    }
    log.warn(`   ⚠️ No open limit order found for token ${tokenId.slice(0, 16)}...`);
  }
  ...
  async executeLimitSell(tokenId: string, tokenType: TokenType, price: number, size: number): Promise<void> {
    if (this.simulation) {
      log.info(`🎮 SIMULATION: Limit sell would place ${tokenTypeDisplayName(tokenType)} ${size} @ $${price.toFixed(2)}`);
      return;
    }
    const client = this.clobClient;
    ...
    log.info(`✅ LIMIT SELL PLACED - ${tokenTypeDisplayName(tokenType)} ${size} @ $${price.toFixed(2)} Order ID: ${result.orderID}`);
  }
}

type LogFn = {
  (msg: string, level?: "debug" | "info" | "warn" | "error"): void;
...
const log: LogFn = ((msg: string, level: "debug" | "info" | "warn" | "error" = "info") => {
  logLine(msg, level);
}) as LogFn;

log.debug = (msg) => logLine(msg, "debug");
log.info = (msg) => logLine(msg, "info");
log.warn = (msg) => logLine(msg, "warn");
log.error = (msg) => logLine(msg, "error");
